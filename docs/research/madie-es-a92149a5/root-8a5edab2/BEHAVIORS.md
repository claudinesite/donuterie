...
- Closed clip-path: `polygon(0% 0%,100% 0%,100% 0%,0% 0%)`. Open state fills the viewport with a translucent burgundy-to-dark-burgundy gradient and subtle backdrop blur.
...
- CTA hover: rose → cream, `translateY(-2px)`, gap `12px → 16px`, 150ms.
...
- Active pill is rose with dark burgundy text. Inactive pills are transparent with 30%-opacity burgundy border; hover applies a 20%-opacity rose fill.
...
- Instagram cards scale their image slightly on hover and show a rose tint/Instagram mark.

## Contact map

- Map controls are click-driven. Zoom buttons adjust the background scale; reset returns to the initial pan/zoom.
- The Madie marker has a repeating rose pulse. Station labels and Metro icons move with the map canvas.
