...
2. `HeroSection` (`#inicio`) — viewport-height image hero with a dark overlay, language pills, Google rating, display headline, descriptive copy, CTA, and rotating scroll cue.
3. `StoryIntro` (`#story`) — cream paper-textured editorial introduction with centered copy.
4. Cream-to-burgundy wave divider — decorative SVG in normal flow.
5. `StoryGallery` (`#historia`) — burgundy grain/pattern section with a four-card draggable/clickable stack and paired narrative copy.
6. Burgundy-to-cream wave divider.
...
10. Cream-to-burgundy wave divider.
11. `ContactSection` (`#contacto`) — burgundy grain section with interactive illustrated map and contact details.
12. `Footer` — dark burgundy logo, copyright, and legal links.
